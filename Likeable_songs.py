...
class StartPage(Frame):
    ...
    def stworzWidgety(self):
        ...
        nazwa_playlisty = Label(start_frame, text="Podaj dokładną nazwę playlisty", bg="slateblue", font=("Arial", 14), width = 40, height=2)
        nazwa_playlisty.place(x = 360, y = 100)
        ID_playlisty = Label(start_frame, text="Podaj łącze do playlisty", bg="slateblue", font=("Arial", 14), width = 40, height=2)
        ID_playlisty.place(x = 360, y = 225)
        self.entry_nazwa_playlisty = Entry(start_frame, font=("Arial", 14))
    ...
    def playlista(self):
        ...
        self.nazwa = self.entry_nazwa_playlisty.get()
        self.ID = self.entry_ID_playlisty.get()      # ID playlisty to ten fragment linku po ukośniku i pytajnikiem
        if self.ID.startswith('https://open.spotify.com/playlist/'):
            self.ID = self.ID[34:].split("?")[0]
        else:
        ...
        playlist = sp.user_playlist(self.nazwa, self.ID)
...
class PageOne(Frame):
    ...
    def stworzWidgety(self):
        ...
        home_image = PhotoImage(file="E:\PYTHONEK\LABY_Programowanie\LABY_BIG PROJECT\home_button2.png")
        ...
        self.right_frame2 = Frame(self, width=500, height=550, bg="powderblue")
        self.right_frame2.grid(row=1, column=3, padx=20)
        self.ramka_proponowane = Entry(self.right_frame2, state=DISABLED)
        self.ramka_proponowane.place(x=50, y=100, width=400, height=400)
    ...
    def wizualizuj_dane(self):
        ...
        if self.licznik_cech == 1:
            ...
            self.barh = FigureCanvasTkAgg(self.fig, master=self.mid_frame2)
            self.barh.get_tk_widget().pack()
        elif self.licznik_cech == 2:
            self.chart.scatter(self.df.loc[self.indeksy, self.wybrane_cechy[0]], self.df.loc[self.indeksy, self.wybrane_cechy[1]], color='b')
            self.scatter = FigureCanvasTkAgg(self.fig, master=self.mid_frame2)
            self.scatter.get_tk_widget().pack()
        elif self.licznik_cech == 3:
            self.chart.scatter(self.df.loc[self.indeksy, self.wybrane_cechy[0]], self.df.loc[self.indeksy, self.wybrane_cechy[1]], self.df.loc[self.indeksy, self.wybrane_cechy[2]], color='b')
            self.scatter = FigureCanvasTkAgg(self.fig, master=self.mid_frame2)
            self.scatter.get_tk_widget().pack()
